...
const getFeedBack = async (req, resp) => {
    ...
    try {
        const findFeedBack = await trainerFeedBackSchema.aggregate([
            ...
            {
                $set: {
                    'reviewedByName': `${'$employerDetails.fullName'}`,
                    'reviewedByDesignation': `${'$employerDetails.designation'}`,
                    'reviewedByImg': `$employerDetails.basicInfo.profileImg`
                }
            },
            {
                $project: {
...
module.exports = {
    ...
    testProfileApi,
    getAllTrainerDetails, UpdatePhoneNumber, trainerSearchHistory, getFeedBack
}