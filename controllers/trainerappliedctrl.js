    ...
    try {
        const updatedTraining = await trainerAppliedTrainingSchema.findOneAndUpdate(
            {
                ...
                'trainingDetails.postedById': employerId
            },
            {
                $set: {
        ...
        if (updatedTraining) {
            ...
            const getAppliedTraining = await trainerAppliedTrainingSchema.find(
                {
                    'trainingDetails.trainingPostDetails.postedById': employerId,
            ...
            if (updatedTraining && getAppliedTraining) {
                resp.status(201).json({ success: true, message: 'Applied status updated successfully', appliedTrainingDetails: getAppliedTraining });
            }
